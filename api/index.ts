// api/index.ts
// Self-contained Vercel serverless entry point — all Express routes included here.
// Vercel cannot bundle imports from parent directories, so everything lives in this file.
...
Construct a JSON response conforming strictly to this format:
{
  ...
  "logic_gates": [
    {
      "condition": { "field": "fieldName", "operator": ">=", "value": 500000, "currency": "INR" },
      "action": { "type": "APPROVAL_ROUTING", "target": "MANAGER_L1", "priority": "HIGH" }
    }
  ],
  "original_rule": "Summarized human logic here",
  "reviewer_insights": [
    { "type": "efficiency", "title": "Title", "description": "Description", "suggested_action": "Apply Fix" }
  ],
  "status": "DRAFT"
}
Answer ONLY with valid JSON.`;
    ...
    return res.json(JSON.parse(jsonText.trim()));
  } catch (error: any) {
    if (error.message?.includes("GROQ_API_KEY")) {
      return res.status(200).json({
        rule_id: "CLM_PRO_005",
        logic_gates: [
          { condition: { field: "claimAmount", operator: ">=", value: "500000", currency: "INR" }, action: { type: "APPROVAL_ROUTING", target: "MANAGER_L1", priority: "HIGH" } },
        ...
        reviewer_insights: [
          { type: "efficiency", title: "Efficiency Optimization", description: "Swap logic order: Checking 'VIP' tier first reduces database calls. Expected gain: 12ms/req.", suggested_action: "Apply Fix" },
          { type: "bias", title: "Potential Bias Detected", description: "The term 'Manager Approval' without an SLA might cause bottlenecks in certain regions.", suggested_action: "View Details" }
        ],
  ...
  try {
    ...
    const systemPrompt =
      agentType === "explainability"
        ? "You are CogniFlow Explainability Assistant. Explain ML models, trace decision logic, audit decisions for bias, and present findings clearly to compliance officers."
        : agentType === "compliance"
        ? "You are Compliance Watchdog. Audit decisions for GDPR, SOC2, ISO 27001, and EU AI Act. Answer regulatory questions clearly."
        : "You are CogniFlow RuleGPT Assistant. Help write, review, optimize and stress-test autonomous business rules and policy logic gates.";
    ...
    if (error.message?.includes("GROQ_API_KEY")) {
      return res.json({ content: "I stand ready to assist you. To enable real-time intelligence, please configure your `GROQ_API_KEY` in the Vercel Environment Variables." });
    }
    if (error.status === 429 || error.message?.includes("429")) {
      return res.status(429).json({ error: "⚠️ API Quota Exceeded. Please wait before trying again." });
    }
...
app.get("/api/analytics/stream", (_req, res) => {
...
// ─── 5. COMPLIANCE ────────────────────────────────────────────────────────────
app.get("/api/compliance/logs", (_req, res) => {
...
app.post("/api/compliance/verify", (req, res) => {
  ...
  setTimeout(() => {
    ...
    return res.json({ status: "success", log_id, hash: `0x${hex}`, verified_at: new Date().toISOString() });
  }, 1000);
});
...
app.post("/api/compliance/audit", async (req, res) => {
  ...
  const CORPUS: Record<string, any> = {
    GDPR_Art22: { source: "GDPR Article 22 — Automated Decision-Making", triggers: ["auto_approve", "auto_reject", "automated", "no human", "fully automated"], severity: "HIGH" },
  ...
  let alerts: any[] = matched.map(([_, r]) => ({
    severity: r.severity,
    regulation_name: r.source.split("—")[0].trim(),
    article: r.source,
    description: `Policy may conflict with: ${r.source}`,
  ...
  try {
    ...
    return res.json({ compliance_score: score, overall_status: status, alerts, rag_sources_used: matched.map(([_, r]) => r.source), summary: parsed.summary || "Analysis complete." });
  } catch (_) {
    const score = Math.max(40, 100 - alerts.length * 12);
    const status = score >= 85 ? "PASSING" : score >= 60 ? "WARNING" : "CRITICAL";
    return res.json({ compliance_score: score, overall_status: status, alerts, rag_sources_used: matched.map(([_, r]) => r.source), summary: `Found ${alerts.length} compliance considerations.` });
  }
});
...
app.post("/api/shadow-reviewer/audit", (req, res) => {
  ...
  logic_gates.forEach((gate: any, i: number) => {
    ...
    PROTECTED_ATTRS.forEach((attr) => {
      if (field.includes(attr) || attr.includes(field)) {
        biasFlags.push({ attribute: attr, field_in_rule: gate.condition?.field || field, severity: ["race", "gender", "religion"].includes(attr) ? "HIGH" : "MEDIUM", recommendation: `Remove or anonymise '${field}'.` });
      }
    });
    if (seenFields[field]) {
      seenFields[field].forEach((j) => { conflictFlags.push({ gate_a_index: j, gate_b_index: i, conflict_type: "OVERLAPPING_CONDITION", description: `Gates ${j} and ${i} both operate on '${field}'.` }); });
  ...
  if (!Array.from(fieldsCovered).some((f) => (f as string).includes("fraud") || (f as string).includes("risk"))) {
    coverageGaps.push({ scenario: "No fraud/risk signal evaluated", missing_condition: "riskScore < 0.85", suggested_fix: "Add risk score check." });
  }
  ...
  return res.json({ rule_id, bias_flags: biasFlags, conflict_flags: conflictFlags, coverage_gaps: coverageGaps, overall_verdict: verdict, bias_score: Math.round(biasScore * 100) / 100, coverage_score: Math.round(coverageScore * 100) / 100 });
});

// ─── 7. OPTIMISATION A/B TEST ─────────────────────────────────────────────────
app.post("/api/optimisation/ab-test", (req, res) => {
  ...
  const recommendation = significant && uplift > 0 ? `Deploy ${winner} — ${Math.abs(uplift).toFixed(1)}% uplift (p=${pValue.toFixed(3)}).` : `No significant difference (p=${pValue.toFixed(3)}). Expand sample.`;
  return res.json({ rule_id_a, rule_id_b, winner, metric, uplift_pct: Math.round(uplift * 100) / 100, p_value: Math.round(pValue * 10000) / 10000, sample_a: sample_size / 2, sample_b: sample_size / 2, recommendation });
});
...
app.post("/api/explainability/analyze", async (req, res) => {
  ...
  decision_path.forEach((step: string) => {
    ...
    Object.keys(context_data).forEach((field) => {
      ...
      if (step.includes(">=")) {
        ...
        if (match) counterfactuals.push({ feature: field, current_value: context_data[field], required_change: `Decrease below ${match[1]}`, threshold_value: parseFloat(match[1]), new_outcome: "AUTO_APPROVE" });
      }
    });
  });
  ...
  let narrative = `Decision for rule ${rule_id} resulted in ${outcome}. Primary driver: '${topDriver}'.`;
  try {
    const ai = getGroqClient();
    const topFactors = Object.entries(normalized).sort((a, b) => b[1] - a[1]).slice(0, 3);
    const response = await ai.chat.completions.create({
      model: "llama-3.3-70b-versatile",
      messages: [{ role: "user", content: `Generate a 2-3 sentence professional audit narrative. Rule: ${rule_id}, Outcome: ${outcome}, Top factors: ${topFactors.map(([k, v]) => `${k}: ${v}`).join(", ")}. Return ONLY the narrative text.` }],
    });
    narrative = response.choices[0]?.message?.content || narrative;
  } catch (_) {}
  return res.json({ rule_id, outcome, narrative, shap_factors: normalized, counterfactuals, key_driver: topDriver, confidence });
});

// ─── 9. DRIFT MONITOR ─────────────────────────────────────────────────────────
app.get("/api/drift-monitor/status", (_req, res) => {
  const hour = Math.floor(Date.now() / 3600000);
  const rng = (seed: number) => { const x = Math.sin(seed + hour) * 10000; return x - Math.floor(x); };
  ...
  const features = featureConfigs.map((feat) => {
    ...
    return { feature: feat.name, psi_score: psiRounded, ks_statistic: Math.min(ksStatistic, 0.99), ks_p_value: Math.round(Math.max(0.001, 1 - ksStatistic * 2) * 1000) / 1000, drift_level: psiRounded > 0.2 ? "CRITICAL" : psiRounded > 0.1 ? "MODERATE" : "STABLE", baseline_mean: feat.baseMean, current_mean: Math.round(feat.baseMean * feat.drift * 100) / 100 };
  });
  const maxPsi = Math.max(...features.map((f) => f.psi_score));
  const overall = maxPsi > 0.2 ? "CRITICAL" : maxPsi > 0.15 ? "HIGH" : maxPsi > 0.1 ? "MEDIUM" : "LOW";
  return res.json({ overall_staleness: overall, staleness_score: Math.round(maxPsi * 10000) / 10000, alert: maxPsi > 0.15, features, auto_trigger_recommendation: maxPsi > 0.15 ? "Auto-trigger rule retraining — significant distribution shift detected." : "Distributions stable. Next check in 24 hours.", last_checked: new Date().toISOString() });
});
...
app.get("/api/graph-memory/trace/:ruleId", (req, res) => {
  ...
  return res.json({ rule_id: ruleId, nodes, edges, path_summary: `Rule ${ruleId} governed by 2 regulations, 2 decisions, 100% outcome traced.` });
});
...
app.post("/api/time-machine/replay", (req, res) => {
  ...
  setTimeout(() =>
    res.json({ rule_id, simulated_outcomes: 12000 + Math.floor(Math.random() * 6000), confidence_score: Math.round((100 - noise * 200) * 10) / 10, approval_rate_baseline: Math.round(baseApproval * 10) / 10, approval_rate_simulated: Math.round(avgApproval * 10) / 10, revenue_delta_pct: Math.round(avgRevenue * 100) / 100, risk_delta_pct: Math.round(avgRisk * 100) / 100, deployment_recommendation: rec, monte_carlo_sample: sample }),
    1500
  );
});

// ─── Health check ─────────────────────────────────────────────────────────────
app.get("/api/health", (_req, res) => res.json({ status: "ok", service: "CogniFlow AI" }));
